...
async function init() {
    ...
    const leavesfree = freeaddresses.map(x => keccak256(x))
    freetree = new MerkleTree(leavesfree, keccak256, { sortPairs: true })
    freebuf2hex = x => '0x' + x.toString('hex')

    console.log(buf2hex(tree.getRoot()))
    console.log(wlbuf2hex(wltree.getRoot()))
    console.log(freebuf2hex(freetree.getRoot()))

    

    let wltime = await contract.methods.WLStartTime().call()
    let Allowtime = await contract.methods.AllowStartTime().call()
    let Freetime = await contract.methods.FreeStartTime().call()
    ...
    } else if(currentTime>Freetime && Freetime>0){
        document.getElementById("salestatus").textContent = "Freelist Sale is Active";
    } else {
...
async function onMint() {
    ...
    let Allowtime = await contract.methods.AllowStartTime().call()
    let Freetime = await contract.methods.FreeStartTime().call()
    // Allowtime = parseInt(Allowtime)
    // wltime = parseInt(wltime)
    let freeMinted = await contract.methods.FreeMint(account).call()
    let mintednfts = await contract.methods.Max_Minted(account).call()
    mintednfts = parseInt(mintednfts)
    ...
    let leaffree = freebuf2hex(keccak256(account))
    let prooffree = freetree.getProof(leaffree).map(x => freebuf2hex(x.data))
    let isValidfree = await contract.methods.isValidFree(prooffree, leaffree).call()

    console.log(proofal)
    console.log(proofwl)
    console.log(prooffree)
    console.log(freeMinted)
    console.log(mintednfts)
    console.log(isValidAllowed)
    console.log(isValidWL)
    console.log(isValidfree)
    ...
    if (currentTime > Allowtime && isValidAllowed ==true && mintednfts < 3) {
        ...
        document.getElementById("myModal").style.display = "block";
        toastr.info('Select How Many You Wants to Mint and Click on Mint Button', "INFO")
    } else if (currentTime > Freetime && isValidfree == true && mintednfts < 3 && freeMinted < 1){
        document.getElementById("myModal").style.display = "block";
        document.getElementById("33").style.display = "none";
        document.getElementById("22").style.display = "none";
        ...
        toastr.error("You have reached maximum free mint limit", "ERROR")
    } else if (currentTime > Freetime && isValidfree == true && mintednfts == 3) {
        toastr.error("You have reached maximum mint limit", "ERROR")
    ...
    document.getElementById('mint').onclick = async function () {
        ...
        if (currentTime >Allowtime){
            ...
            let freeMinted = await contract.methods.FreeMint(account).call()
            if (freeMinted == 1) return toastr.error("You have reached maximum free mint limit", "ERROR");
            console.log(Date.now() / 1000)
            console.log(parseInt(Freetime))
            
            console.log('test3')
            const leaf = freebuf2hex(keccak256(account))
            const proof = freetree.getProof(leaf).map(x => freebuf2hex(x.data))

            console.log(leaf)
            console.log(proof)
            // var val = (vall).toLocaleString('fullwide', { useGrouping: false });
            await contract.methods.mint(mintCount, proof).send({ from: account , gaslimt:500000})
                .on('transactionHash', function (hash) {
                    console.log(hash);
                    tnxHash = hash;
                })
