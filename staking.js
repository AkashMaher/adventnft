...
async function onClaim() {
    if (!account) return alert(`please connect wallet first`);
        ...
        await contract.methods.claim(stakedNFTs).send({ from: account })
            ...
            console.log(tnxHash)
            document.querySelector("#tnx").addEventListener("click", onTnx(tnxHash));
            document.getElementById("tnx").style.display = "block";
            alert('Success!, Tokens Claimed Successfully')
            

    }


    async function onTnx(a){
        
        window.open(
            "https://rinkeby.etherscan.io/tx/" + a, "socialPopupWindow", "location=no,width=600,height=600,scrollbars=yes,top=100,left=700,resizable = no");
    }
